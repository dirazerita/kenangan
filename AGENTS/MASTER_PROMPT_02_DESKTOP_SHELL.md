...
## LOCAL DB (SQLDelight)
Tables mirroring MEMORY §6: `project(id, name, ratio, vibe, tier, narration, music_path, status, created_at, updated_at)`, `photo(id, project_id, local_path, upload_id?, analysis_json?)`, `scene(...Scene contract fields..., local_keyframe_path?, local_clip_path?)`, `job(id, scene_id, backend_job_id, status, error_code?)`, `output(id, project_id, path, ratio, tier, est_cost_usd, created_at)`, `gen_cost(project_id, job_id, model, key_label, qty, unit, est_usd, at)` (CostTracker; `key_label` = which fal key served the call, AD-14), `settings(key, value)` (keys reserved for the future LicenseState cache + trial counter — unused while `LicenseGate` is stubbed). All writes in transactions; app must resume any project after force-kill.
...
Secure storage: Windows Credential Manager via JNA (`CredWrite/CredRead`), AES-GCM encrypted-file fallback (per-machine derived key); values masked in UI (`fal_…a1b2`), never logged, never synced. Fields: **fal (wajib) — an ORDERED LIST `[{label, key}]` (AD-14)**: UI supports add/remove/reorder, per-key status chip (aktif / cadangan / saldo habis), and "Tes koneksi" per key; Google Gemini (opsional, single key — "analisis lebih tajam"); ElevenLabs (opsional, single key — "suara premium"). Each key has **Tes koneksi**: fal → cheapest 1-token LLM ping (show "biaya tes < $0.001"); Gemini → `models.list` (free); ElevenLabs → `GET /voices` (free). Onboarding shows the anti-credit-farming notice (MEMORY §7): multi-key is for keys the user legitimately owns; farming fal signup credits violates fal ToS. First-run **onboarding wizard**: 3 guided steps with screenshots, "Buka halaman pembuatan key" browser buttons, paste field, live test, and the BYOK responsibility notice (MEMORY §7); skippable, reopenable from Settings.

## PROVIDER CLIENT LAYER (`core/providers/`)
Foundation for Phases 03–04 (there is no backend proxy):
- `KeyVault` (above; fal = ordered key list) · `PriceBook` (loads config price_hints + fx_idr from ConfigRepository — bundled `app-config.json` during development, identical schema to the future remote config; `estimate(modelSlug, qty)`) · `CostTracker` (writes `gen_cost` incl. `key_label`, exposes per-project, per-month & per-key sums; Settings shows this-month spend per key).
- `FalQueueClient` (**multi-key failover, AD-14**): submit uses the first non-exhausted key from the ordered list; on fal's balance-exhausted error (`User is locked. Reason: TOP_UP` / `Exhausted balance` — exact signatures in poc/results.csv) → mark that key exhausted (timestamp, 10-minute cooldown before it may be retried), emit a `KeySwitched` event (UI toast: "Saldo key 'X' habis — beralih ke 'Y'"), and retry the submit once with the next key; if ALL keys are exhausted/cooling → `provider_balance` AppError with CTA "Top up / kelola key". **Polling always uses the key stored on the job** (in-flight jobs are never migrated between keys). Mechanics: submit/status/result against fal queue endpoints (slugs from config), exponential backoff, error mapping → `AppError { content_blocked | invalid_key(401) | provider_balance(402) | rate_limited(429) | provider_failed | timeout }`.
