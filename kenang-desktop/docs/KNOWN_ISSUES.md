...
| KI-018 | P2 | Wizard | Bundled music shipped at 27 tracks, not the requested 50: the installer hard cap (<300 MB, MEMORY §5) leaves ~165 MB for music and 50 tracks need ~190 MB; also 29/56 candidate CC-BY URLs 404'd. Reaching 50+ needs a download-on-demand track store fed by remote config (Phase 01+) | OPEN (design decision D-014) |
| KI-016 | P2 | Settings | "Tes koneksi" reads the fal credit balance; the owner's ORIGINAL keys answered 403 (admin-scope only). Update 2026-08-28: the owner's newer keys DO return balances, and the new "Cek Saldo" button sweeps all keys and sums the readable ones (verified live: 6 keys, 5.00 USD each). Keys that still 403 are counted as "tidak terbaca", never as failures | RESOLVED in practice (per-key scope dependent) |
