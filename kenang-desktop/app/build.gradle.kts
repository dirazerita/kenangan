...
tasks.register<JavaExec>("upscaleDoctor") {
    ...
    providers.gradleProperty("doctorPadOnly").orNull?.let { systemProperty("doctor.padOnly", it) }
}
...
tasks.register<JavaExec>("talkingDoctor") {
    ...
    providers.gradleProperty("doctorAudioOnly").orNull?.let { systemProperty("doctor.audioOnly", it) }
}
