    ...
    Column(Modifier.fillMaxSize().padding(24.dp).verticalScroll(rememberScrollState())) {
        ...
        FlowRow(horizontalArrangement = Arrangement.spacedBy(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            if (items.any { it.status == ItemStatus.DONE }) {
                SkeuoOutlinedButton(onClick = {
                    runCatching { java.awt.Desktop.getDesktop().open(service.outputDir()) }
...
/** Multi-select image picker — same JFileChooser pattern as the wizard (KI-013). */
private fun pickImages(): List<File> {
    val chooser = javax.swing.JFileChooser().apply {
        dialogTitle = Strings.UPSCALE_PICK
