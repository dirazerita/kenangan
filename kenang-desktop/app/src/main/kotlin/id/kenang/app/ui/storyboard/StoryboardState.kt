...
class StoryboardState(
     ...
     * storyboard shows exactly what the video will show). Unreadable formats
     * (e.g. webp without an ImageIO plugin) are left untouched.
     */
    ...
    fun addAiScene() {
        scope.launch {
            ...
            // Template: prefer a normal photo-derived scene ("custom" rows
            // have no source photos to generate from).
     ...
     * never worth blocking the dialog for, so any provider trouble just
     * yields the old kind of idea instead of an error.
     */
