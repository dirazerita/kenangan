...
fun StoryboardScreen(
    ...
    // Phone layout: the desktop header packs title + estimate + both buttons
    // into one Row; here it stacks so nothing is pushed off-screen.
    Column(Modifier.fillMaxSize().padding(16.dp)) {
        // ---------- Header ----------
        Row(verticalAlignment = Alignment.CenterVertically) {
            Column(Modifier.weight(1f)) {
                Text(
                    state.project?.name ?: Strings.SB_TITLE_FALLBACK,
    ...
    if (showAddScene) {
        AddSceneDialog(
            photo = addScenePhoto,
            onPickPhoto = { replaceTarget = null; launchPicker() },
            onAdd = { file, category, camera, description ->
                state.addUserScene(file, category, camera, description)
                addScenePhoto = null
                showAddScene = false
            },
            onDismiss = { addScenePhoto = null; showAddScene = false },
        )
    }
...
// Image picking uses the Android photo picker (launcher in StoryboardScreen).
...
/**
 * "Ubah deskripsi adegan" (owner 2026-09-06 rev 2): the edited Indonesian
 * text becomes AUTHORITATIVE for the image on the next (re)generation.
 */
@Composable
