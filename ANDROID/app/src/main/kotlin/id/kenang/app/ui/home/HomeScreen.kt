...
fun HomeScreen(
    ...
    // Phone layout: the desktop header is one wide Row, which pushed the
    // settings/about buttons off a 1080px-wide screen — the icons come first
    // on their own line so API keys are always reachable.
...
private fun produceThumbnail(path: String?) =
    // Home shows small cards — 480px is plenty and keeps a long project list
    // off the heap ceiling.
    id.kenang.app.ui.components.rememberFileBitmap(path, maxSide = 480)
