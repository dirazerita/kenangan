        ...
        when (val o = outcome) {
            ...
            is AnalysisOutcome.Failed -> Column(horizontalAlignment = Alignment.CenterHorizontally) {
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    // No guaranteed file manager on a phone — copying the
                    // technical detail is what actually helps the user report it.
                    val ctx = androidx.compose.ui.platform.LocalContext.current
                    var copied by remember { mutableStateOf(false) }
                    androidx.compose.material3.TextButton(onClick = {
                        id.kenang.app.ui.platform.AndroidActions.copyToClipboard(
                            ctx, "Kenang error", o.error.toString(),
                        )
                        copied = true
                    }) { Text(if (copied) Strings.KEYS_LINK_COPIED else Strings.KEYS_COPY_LINK) }
                }
            }
