...
object Strings {
    ...
    // Video Berbicara (owner 2026-09-13): a photo speaks a script.
    const val TALK_TITLE = "Video Berbicara"
    const val TALK_SUBTITLE = "Foto berbicara sesuai naskah Anda — suaranya dikloning dari file rekaman, atau pilih suara yang tersedia. Memakai OmniHuman 1.5 (ByteDance), model foto-berbicara terbaik di fal.ai."
    ...
    const val TALK_THEME_HINT = "Mis. salam perpisahan dari almarhumah ibu untuk anak-anaknya, atau pesan ulang tahun untuk cucu"
    const val TALK_THEME_LENGTH = "Panjang"
    const val TALK_WRITE = "✨ Buatkan naskah"
    const val TALK_REWRITE = "✨ Buat ulang"
    const val TALK_WRITING = "AI sedang menulis naskah…"
    const val TALK_NEED_THEME = "Tulis dulu tema ucapannya."
    const val TALK_AI_NOTE = "Naskah dibuat dari tema Anda (dan foto yang dipilih, jika ada) — biayanya hanya beberapa sen. Hasilnya masih bisa Anda edit sendiri."
    const val TALK_SCRIPT_LABEL = "Naskah yang diucapkan"
    const val TALK_SCRIPT_HINT = "Tulis apa yang harus diucapkan… (maks %1 karakter ≈ 60 detik)"
    const val TALK_VOICE_TITLE = "Suara"
    const val TALK_VOICE_PRESET = "Suara pilihan"
    const val TALK_VOICE_FILE = "Dari file suara (kloning)"
    const val TALK_VOICE_FILE_PICK = "Pilih file suara (WAV/MP3/M4A, minimal 10 detik)"
    const val TALK_VOICE_FILE_NOTE = "Suara di video akan meniru suara di file ini. Kloning ±$%1 sekali per file — file yang sama dipakai ulang tanpa biaya."
    const val TALK_VOICE_CLONED_TAG = "  ✓ sudah dikloning"
    const val TALK_VOICE_PLAY = "▶ Dengar"
    const val TALK_VOICE_STOP = "■ Berhenti"
    const val TALK_VOICE_LOADING = "… memuat"
    const val TALK_VOICE_PREVIEW_FAILED = "Contoh suara gagal diputar"
    const val TALK_MODEL_LABEL = "Model"
    const val TALK_START = "Buat Video Berbicara"
    const val TALK_NEED_INPUT = "Pilih foto dan tulis naskahnya dulu."
    const val TALK_NEED_VOICE_FILE = "Pilih file suaranya dulu, atau ganti ke Suara pilihan."
    const val TALK_PHASE_CLONING = "Mengkloning suara dari file…"
    const val TALK_PHASE_SPEAKING = "Membuat suara dari naskah…"
    const val TALK_PHASE_RENDERING = "Membuat video berbicara… (±5–10 menit, biarkan terbuka)"
    const val TALK_PHASE_SAVING = "Menyimpan hasil…"
    const val TALK_DONE_PREFIX = "Selesai! Video tersimpan di: "
    ...
    const val SB_MAKE_SCENE_VIDEO = "🎬 Buat video adegan"
    ...
    const val SB_EDIT_DESC_NOTE = "Deskripsi ini MENGIKAT gambar adegan — termasuk jumlah orangnya (mis. \"Mereka bertiga\" → \"Mereka berdua\"). Setelah diubah, klik \"Buat ulang gambar\". Kembalikan ke teks semula untuk membatalkan."
    ...
    const val WIZARD_PICK_FROM_GALLERY = "Ketuk untuk memilih foto dari galeri"
    ...
    // Phone wording: there is no Explorer to drag from — the picker opens.
    const val SB_REPLACE_IMAGE_HINT = "Ketuk untuk memilih foto dari galeri"
    ...
    const val SB_ADD_AIREF_DESC_HINT = "Kosongkan untuk memakai usulan — atau tulis adeganmu sendiri…"
    const val SB_ADD_AIREF_SUGGEST_PREFIX = "💡 Usulan adegan: "
    const val SB_ADD_AIREF_SUGGEST_SWAP = "Ganti usulan"
    const val SB_ADD_AIREF_SUGGEST_NOTE = "Usulan dibaca dari foto yang Anda pilih dan disambungkan dengan cerita — tidak mengulang adegan yang sudah ada."
    const val SB_ADD_AIREF_SUGGEST_LOADING = "💡 Membaca foto dan menyusun usulan adegan…"
    ...
    const val CRASH_TITLE = "Terjadi kesalahan tak terduga"
    const val CRASH_BODY = "Aplikasi tetap berjalan: tampilan dimuat ulang, proyek dan pekerjaan yang sudah dikirim ke penyedia tetap aman. Rinciannya tersimpan di log aplikasi."
    const val CRASH_OK = "Tutup"
    const val GEN_ELAPSED_PREFIX = "Berjalan "
    const val GEN_WAITING_SLOT = "Menunggu giliran"
    const val GEN_QUEUE_AT_PROVIDER = "Antre di penyedia"
    const val GEN_QUEUE_POSITION = "posisi %1"
    const val GEN_DOWNLOADING = "Mengunduh klip"
    const val GEN_ETA = "±%1 lagi"
    const val GEN_ATTEMPT = "percobaan ke-%1"
    ...
    const val RESULT_OPEN_GALLERY = "Buka Galeri"
    const val RESULT_GALLERY_NOTE = "Video otomatis tersimpan di Galeri: Movies/Kenang/<nama proyek>. Klip per adegan ada di subfolder Adegan."
    const val WIZARD_PICK_MUSIC = "Pilih musik dari HP"
    const val SET_OUTPUT_ANDROID = "Hasil video otomatis masuk ke Galeri (Movies/Kenang). Tidak perlu mengatur folder."
}
