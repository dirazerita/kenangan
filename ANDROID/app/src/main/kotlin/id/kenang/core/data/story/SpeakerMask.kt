...
object SpeakerMask {
    ...
    fun render(
        ...
        return runCatching {
            ...
            fun paint(region: List<Double>, argb: Int) {
                val left = (region[0] * width).toFloat().coerceIn(0f, (width - 1).toFloat())
                val top = (region[1] * height).toFloat().coerceIn(0f, (height - 1).toFloat())
                val right = (region[2] * width).toFloat().coerceIn(left + 1f, width.toFloat())
                val bottom = (region[3] * height).toFloat().coerceIn(top + 1f, height.toFloat())
                canvas.drawRect(left, top, right, bottom, Paint().apply { color = argb })
            }
